...
def prepare_data_multiwoz(args):
    ...
    elif args["domain_option"] == "zero": #eval on 4 domains, and eval on the remaining domains
        print(args["domain"], args["domain_option"])
        file_trn = os.path.join(args["data_path"], 'MultiWOZ-{}/zero-shot/train/{}_train_dials.json'.format(version, args["domain"]))
        file_dev = os.path.join(args["data_path"], 'MultiWOZ-{}/zero-shot/dev/{}_dev_dials.json'.format(version, args["domain"]))
        file_tst = os.path.join(args["data_path"], 'MultiWOZ-{}/test/{}_test_dials.json'.format(version, args["domain"]))
    elif args["domain_option"] == "full":
        print(args["domain"], args["domain_option"])
        file_trn = os.path.join(args["data_path"], 'MultiWOZ-{}/train_dials.json'.format(version))
        file_dev = os.path.join(args["data_path"], 'MultiWOZ-{}/dev_dials.json'.format(version))
        file_tst = os.path.join(args["data_path"], 'MultiWOZ-{}/test/{}_test_dials.json'.format(version, args["domain"]))

...
class BeliefTracker(nn.Module):
    def __init__(self, args):
        ...
        ### Utterance Encoder
        print("Update")
        # Load pretrained-transformer (i.e. without adapters)
        ...
        if args['do_train']:
            ...
            self.utterance_encoder.load_adapter(args['adapter_name_or_path'], config="pfeiffer", load_as = 'MLM')       
            # Train adapter and unfreeze all weights
            self.utterance_encoder.train_adapter(['MLM'])
            self.utterance_encoder.freeze_model(freeze=False)
            # Set the adapters to be used in every forward pass
            self.utterance_encoder.set_active_adapters(['MLM'])
        else:
            print("Update")
            adapter_name = self.utterance_encoder.load_adapter(args['adapter_name_or_path'])
            self.utterance_encoder.set_active_adapters(adapter_name)
        print(self.utterance_encoder)

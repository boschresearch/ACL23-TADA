...
from .meta_model import BertEmbed, BertMetaDomainEmbed
...
class BeliefTracker(nn.Module):
    def __init__(self, args):
        ...
        tokenizer_1 = AutoTokenizer.from_pretrained(self.args["model_name_or_path"], cache_dir = args["cache_dir"])
        tokenizer_2 = AutoTokenizer.from_pretrained(self.args["model_name_or_path_2"], cache_dir = args["cache_dir"])
        tokenizer_3 = None if self.args["model_name_or_path_3"]==None else AutoTokenizer.from_pretrained(self.args["model_name_or_path_3"], cache_dir = args["cache_dir"])
        tokenizer_4 = None if self.args["model_name_or_path_4"]==None else AutoTokenizer.from_pretrained(self.args["model_name_or_path_4"], cache_dir = args["cache_dir"])
        tokenizer_5 = None if self.args["model_name_or_path_5"]==None else AutoTokenizer.from_pretrained(self.args["model_name_or_path_5"], cache_dir = args["cache_dir"])
        tokenizer_6 = None if self.args["model_name_or_path_6"]==None else AutoTokenizer.from_pretrained(self.args["model_name_or_path_6"], cache_dir = args["cache_dir"])
        
        logging.info("embedding/tokenizer 1: {}".format(self.args["model_name_or_path"]))
        logging.info("embedding/tokenizer 2: {}".format(self.args["model_name_or_path_2"]))
        logging.info("embedding/tokenizer 3: {}".format(self.args["model_name_or_path_3"]))
        logging.info("embedding/tokenizer 4: {}".format(self.args["model_name_or_path_4"]))
        logging.info("embedding/tokenizer 5: {}".format(self.args["model_name_or_path_5"]))
        logging.info("embedding/tokenizer 6: {}".format(self.args["model_name_or_path_6"]))

        self.utterance_encoder.embeddings = BertMetaDomainEmbed(config = self.utterance_encoder.config, 
        ...
        logging.info("Subword aggregation method: {}".format(self.utterance_encoder.embeddings.method))
...
#                 param[1].requires_grad=False
    ...
    def initialize_slot_value_lookup(self, max_seq_length=32):
        ...
        #logging.info("label_ids: {}".format(label_ids))
        #logging.info("len(label_ids): {}".format(len(label_ids)))
...
#             logging.info("Param: {} Requires_grad: {}".format(param[0], param[1].requires_grad))
        ...
        #logging.info("num_slots: {}".format(self.num_slots))
        if self.training: 
            self.loss_grad = loss
            self.optimize()
